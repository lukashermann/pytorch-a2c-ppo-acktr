...
def train(sysargs):
    ...
    cfg.training_name = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M") + "_seed-{}".format(
        cfg.globals.seed)
    if cfg.experiment.tag is not None:
        cfg.log_dir = os.path.join(cfg.experiment.root_dir, cfg.experiment.tag, cfg.training_name)
    else:
        cfg.log_dir = os.path.join(cfg.experiment.root_dir, cfg.training_name)
    ...
    eval_log_dir = os.path.join(cfg.log_dir, "_eval")
    ...
    if cfg.experiment.snapshot is None:
        ...
        load_data = torch.load(cfg.experiment.snapshot)
    ...
    for j in tqdm(range(num_updates), desc="Updates"):
        ...
        if cfg.learning.optimizer.use_linear_lr_decay:
            ...
            if cfg.learning.rl.algo == "acktr":
                ...
                update_linear_schedule(agent.optimizer, j, num_updates, cfg.learning.optimizer.lr)

        elif cfg.learning.optimizer.use_linear_lr_decay_less and cfg.learning.rl.algo == "ppo":
            update_linear_schedule_less(agent.optimizer, j, num_updates, cfg.learning.optimizer.lr)

        elif cfg.learning.optimizer.use_linear_lr_decay_half and cfg.learning.rl.algo == "ppo":
            update_linear_schedule_half(agent.optimizer, j, num_updates, cfg.learning.optimizer.lr)
        ...
        if cfg.learning.rl.algo == 'ppo' and cfg.learning.rl.ppo.use_linear_clip_decay:
            agent.clip_param = cfg.learning.rl.ppo.clip_param * (1 - j / float(num_updates))
        elif cfg.learning.rl.algo == 'ppo' and cfg.learning.rl.ppo.use_linear_clip_decay_less:
            agent.clip_param = cfg.learning.rl.ppo.clip_param * (1 - j / float(2 * num_updates))
        ...
        if (j % cfg.experiment.save_interval == 0 or j == num_updates - 1) and cfg.save_dir != "":
            ...
            torch.save(save_model, os.path.join(save_path, cfg.env.name + "_" + str(j) + ".pt"))
        ...
        if j % cfg.experiment.log_interval == 0 and len(episode_rewards) > 1:
            ...
            log_file.flush()
        if (cfg.experiment.eval_interval is not None
        ...
        if cfg.experiment.vis and j % cfg.experiment.vis_interval == 0:
            try:
                ...
                win = visdom_plot(viz, win, cfg.log_dir, cfg.training_name,
        ...
        if cfg.tensorboard and len(curr_episode_rewards) > 1:
            ...
            tb_writer.add_scalar("regular_resets_ratio",
                                 ...
                                 total_num_steps)
        if cfg.tensorboard and len(reg_episode_rewards) > 1:
